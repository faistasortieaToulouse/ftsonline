// src/app/museetarn/page.tsx
'use client';

import { useEffect, useState, useRef, useCallback } from 'react';
// Importe le type depuis l'API du Tarn (81)
...
const GoogleMap = ({ musees }: { musees: Musee[] }) => {
  ...
  const initMap = useCallback(() => {
    ...
    // Le centre de la carte n'est calculé que si des musées sont présents.
    if (!mapRef.current || !window.google || musees.length === 0) return;

    // Calculer le centre de la carte (Tarn - estimation basée sur la moyenne des coordonnées)
    ...
    const map = new window.google.maps.Map(mapRef.current, {
      ...
      zoom: 9, // Zoom initial pour le Tarn
      scrollwheel: true,
    });
...
export default function MuseeTarnPage() {
  ...
  useEffect(() => {
    async function fetchMusees() {
      try {
        // L'API est déjà triée par commune (A-Z) par défaut
