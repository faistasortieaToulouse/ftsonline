...
export default function AveyronMapPage() {
        ...
        L.marker([site.lat, site.lng], { icon: customIcon })
          .bindPopup(`
            <div style="text-align: center; font-family: sans-serif; max-width: 200px;">
              ...
              <a href="#site-aveyron-${currentNum}" style="display: inline-block; background-color: ${color}; color: white; padding: 4px 8px; border-radius: 4px; text-decoration: none; font-size: 11px; font-weight: bold;">Voir dans la liste ↓</a>
            </div>
          `)
  ...
  return (
    <div className="max-w-7xl mx-auto p-4 bg-slate-50 min-h-screen">
      <nav className="mb-4">
        <Link href="/" className="inline-flex items-center gap-2 text-rose-800 font-bold hover:underline">
      ...
      <header className="mb-6">
        <div className="flex flex-col md:flex-row md:items-end justify-between gap-4">
          <div>
            <h1 className="text-2xl md:text-3xl font-extrabold text-slate-900 italic leading-tight">🗺️ Sites Touristiques en Aveyron (12)</h1>
            <div className="flex items-center gap-2 text-rose-700 font-bold text-sm mt-2">
              <Database size={16} />
      ...
      <div className="relative mb-6">
        ...
        <input 
          type="text"
          placeholder="Rechercher une commune, un viaduc, un village..."
          className="w-full pl-10 pr-4 py-3 rounded-xl border border-slate-200 focus:ring-2 focus:ring-rose-600 outline-none shadow-sm transition-all text-sm"
          value={searchQuery}
        ...
        <table className="w-full text-left border-collapse text-sm table-fixed md:table-auto">
          ...
          <tbody className="divide-y divide-slate-100">
            {filteredSites.map((site, i) => (
              <React.Fragment key={`aveyron-${site.id}`}>
                <tr 
                  id={`site-aveyron-${i + 1}`}
                  onClick={() => setExpandedId(expandedId === i ? null : i)}
                  className={`cursor-pointer transition-colors scroll-mt-16 ${expandedId === i ? 'bg-rose-50/30' : 'hover:bg-slate-50'}`}
                >
                ...
                {expandedId === i && (
                  <tr className="bg-rose-50/20 md:hidden">
                    <td colSpan={3} className="p-4 pt-0">
                      <div className="flex flex-col gap-3 py-4 border-t border-rose-100">
                        ...
                        <div className="flex items-center gap-2 text-slate-500">
                          <MapPin size={16} />
                          <span className="text-xs italic">Département de l'Aveyron (12)</span>
                        </div>
                      </div>
                    </td>
                  </tr>
