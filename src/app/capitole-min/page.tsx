...
export default function CapitoleMinPage() {
  ...
  useEffect(() => {
    if (!searchQuery) return setFilteredEvents(events);
  ...
  return (
    <div className="container mx-auto py-10 px-4 sm:px-6 lg:px-8">
      ...
      <p className="text-muted-foreground mb-6">
        Événements filtrés depuis le site officiel de l’Université Toulouse Capitole.
      </p>
          ...
          {filteredEvents.map(ev => (
            <div key={ev.id} className="bg-white shadow rounded overflow-hidden flex flex-col h-[420px]">
              ...
              <div className="p-4 flex flex-col flex-1">
                ...
                {ev.start && <p className="text-sm text-blue-600 font-medium mb-2">{ev.start}</p>}
                {ev.location && <p className="text-sm text-muted-foreground mb-2">📍 {ev.location}</p>}
                <div className="text-sm text-muted-foreground overflow-y-auto h-20 mb-2 pr-1 scrollable">{ev.description}</div>
                <a href={ev.url} target="_blank" rel="noopener noreferrer" className="text-blue-600 hover:underline text-sm mb-2">🔗 Plus d’informations</a>
        ...
        <div className="flex flex-col gap-4">
          {filteredEvents.map(ev => (
            <div key={ev.id} className="flex flex-col sm:flex-row bg-white shadow rounded p-4 gap-4">
              <img src={getEventImage(ev.title)} alt={ev.title} className="w-full sm:w-48 h-32 object-cover rounded" />
              <div className="flex-1">
                <h2 className="text-lg font-semibold mb-1">{ev.title}</h2>
                {ev.start && <p className="text-sm text-blue-600 font-medium mb-1">{ev.start}</p>}
                ...
                <a href={ev.url} target="_blank" rel="noopener noreferrer" className="text-blue-600 hover:underline text-sm">🔗 Plus d’informations</a>
