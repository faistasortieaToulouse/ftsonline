...
export default function MuseeGersPage() {
  ...
  return (
    <div className="max-w-7xl mx-auto p-4 bg-slate-50 min-h-screen font-sans">
      <nav className="mb-6">
        <Link href="/" className="inline-flex items-center gap-2 text-red-700 font-bold hover:underline group">
      ...
      <header className="mb-8">
        <h1 className="text-3xl font-black text-slate-900 leading-tight uppercase tracking-tight italic">
          🍷 Patrimoine du Gers (32)
        </h1>
        <p className="text-slate-500 font-medium italic mt-1">
          {isLoadingData ? 'Chargement des données...' : `Découvrez ${filteredMusees.length} joyaux de la Gascogne.`}
        </p>
      </header>
      ...
      <h2 className="text-2xl font-black mb-6 text-slate-800 flex items-center gap-3">
        ...
        INVENTAIRE GASCON ({filteredMusees.length})
      </h2>
          ...
          <tbody className="divide-y divide-slate-100">
            {filteredMusees.map((m, i) => (
              <React.Fragment key={`group-${i}`}>
                ...
                >
                  <td className="p-5 text-center font-black text-red-600 align-top">{i + 1}</td>
                  ...
                  <td className="p-5 align-top">
                    ...
                    <div className="text-[10px] text-red-600 font-black md:hidden mt-1 uppercase italic tracking-wider">{m.commune}</div>
                  </td>
                  <td className="p-5 hidden md:table-cell align-top text-center">
                    <span className="px-2 py-1 bg-red-100 text-red-700 rounded-lg text-[9px] font-black uppercase tracking-tighter">
                        {m.categorie}
                    </span>
                  </td>
      ...
      <footer className="py-10 border-t border-slate-200 text-center">
        <p className="text-[10px] font-black text-slate-300 uppercase tracking-[0.6em]">
            Gers Tourisme • 2026 • Esprit Gascogne
        </p>
      </footer>
    </div>
  );
