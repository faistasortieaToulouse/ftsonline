        ...
        const episodes: PodcastEpisode[] = feed.items
            .map(item => {
                ...
                return {
                    ...
                    titre: item.title,
                    date: item.pubDate,
                    // Utilisation de content si content:encoded n'est pas disponible
                    description: item.contentSnippet || item.content || 'Pas de description disponible', 
...
export async function GET() {
    ...
    try {
        // Lancer toutes les récupérations de flux en parallèle
        const fetchPromises = Object.entries(PODCAST_FEEDS).map(([librairieName, feedUrl]) => 
