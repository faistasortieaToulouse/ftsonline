...
const PLACEHOLDER_IMAGE = "[https://via.placeholder.com/400x200?text=Événement](https://via.placeholder.com/400x200?text=Événement)";
...
// -------------------------
// ⚡ Fetch JSON sécurisé avec retry
// -------------------------
...
for (const route of meetupRoutes) {
  ...
  try {
    // ⚠️ Utiliser l'URL de production ou import direct
    const data = await fetchWithRetry(`https://ton-app.vercel.app/api/${route}`);
    meetupCache[route] = { timestamp: now, data };
...
const allEvents = results.flatMap(({ route, data }) => {
  ...
  return listToNormalize.map(ev => normalizeEvent(ev, route)).filter(Boolean);
});

// 4️⃣ Filtrage 31 prochains jours
const now = new Date();
