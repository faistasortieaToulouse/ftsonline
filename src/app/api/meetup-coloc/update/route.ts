...
export async function GET() {
  try {
    ...
    finalEvents = finalEvents.filter(e =>
      e.startDate >= startFilter && e.startDate < endFilter
    );
    ...
    // 5️⃣ Cache côté serveur
    const futureTriggers = REFRESH_TIMES
      .map(t => {
