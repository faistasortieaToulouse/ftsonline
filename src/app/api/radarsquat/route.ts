...
export async function GET(request: NextRequest) {
  try {
    ...
    const EXTERNAL_SOURCES = [
      ...
      { url: `${origin}/api/radarsquat`, source: "Radar Squat Toulouse" },
    ];
    ...
    const results = await Promise.all(
      EXTERNAL_SOURCES.map(async ({ url, source }) => {
        try {
          ...
          // 🏴‍☠️ TRAITEMENT RADAR SQUAT TOULOUSE
          if (source === "Radar Squat Toulouse") {
            return items.map((ev: any) => ({
              ...ev,
              date: ev.start, // On remap start vers date pour la normalisation finale
              source
            }));
          }
