...
const PhilibertnetPage: React.FC = () => {
  ...
  useEffect(() => {
    async function fetchData() {
      try {
        const response = await fetch('/api/philibertnet', { cache: 'no-store' });
  ...
  if (loading)
    return (
      <main className="p-8">
        <h1>🛍️ Chargement du flux Philibert…</h1>
      </main>
    );
  ...
  if (data?.error) {
    return (
      <main className="p-8">
        ...
        <p className="mt-4"><strong>{data.error}</strong></p>
        {data.details && <p className="mt-2 text-sm italic text-gray-700">Détails : {data.details}</p>}
      </main>
    );
  }
  ...
  return (
    <main className="p-8">
      <h1 className="text-3xl font-bold mb-2">{data?.title || 'Nouveautés Philibert'}</h1>
      <p className="mb-6 text-gray-600 italic">Source : {data?.source} — Articles : {data?.count}</p>
        ...
        {data?.items.map((item, index) => (
          <article key={index} className="border-l-4 border-green-600 pl-4 pb-4">
            ...
            <p className="text-sm text-gray-500 mt-1">
              Publié le {item.pubDate ? new Date(item.pubDate).toLocaleDateString('fr-FR') : 'date inconnue'}
            </p>
