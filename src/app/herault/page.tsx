...
const getMarkerColor = (categorie: SiteHerault['categorie']): string => {
  switch (categorie) {
...
export default function HeraultMapPage() {
  ...
  useEffect(() => {
    async function fetchSites() {
      try {
        ...
        // Normalisation des catégories (Majuscule)
        const formattedData: SiteHerault[] = data.map((item: any) => ({
          ...item,
          categorie: (item.categorie.charAt(0).toUpperCase() + item.categorie.slice(1)) as SiteHerault['categorie']
        }));
  ...
  return (
    <div className="max-w-7xl mx-auto p-4 bg-slate-50 min-h-screen">
      <nav className="mb-4">
        <Link href="/" className="inline-flex items-center gap-2 text-cyan-800 font-bold hover:underline">
      ...
      <header className="mb-6">
        <div className="flex flex-col md:flex-row md:items-end justify-between gap-4">
          <div>
            <h1 className="text-2xl md:text-3xl font-extrabold text-slate-900 italic leading-tight">🌊 Patrimoine de l'Hérault (34)</h1>
            <div className="flex items-center gap-2 text-cyan-700 font-bold text-sm mt-2">
              <Database size={16} />
      ...
      <div className="relative mb-6">
        ...
        <input 
          type="text"
          placeholder="Rechercher une abbaye, un cirque, une cité balnéaire..."
          className="w-full pl-10 pr-4 py-3 rounded-xl border border-slate-200 focus:ring-2 focus:ring-cyan-600 outline-none shadow-sm transition-all text-sm"
          value={searchQuery}
        ...
        <table className="w-full text-left border-collapse text-sm table-fixed md:table-auto">
          ...
          <tbody className="divide-y divide-slate-100">
            {filteredSites.map((site, i) => (
              <React.Fragment key={`herault-${site.id}`}>
                <tr 
                  onClick={() => setExpandedId(expandedId === i ? null : i)}
                  className={`cursor-pointer transition-colors ${expandedId === i ? 'bg-cyan-50/30' : 'hover:bg-slate-50'}`}
                >
                ...
                {expandedId === i && (
                  <tr className="bg-cyan-50/20 md:hidden">
                    <td colSpan={3} className="p-4 pt-0">
                      <div className="flex flex-col gap-3 py-4 border-t border-cyan-100">
                        ...
                        <div className="flex items-center gap-2 text-slate-500">
                          <MapPin size={16} />
                          <span className="text-xs italic">Département de l'Hérault (34)</span>
                        </div>
                      </div>
                    </td>
                  </tr>
