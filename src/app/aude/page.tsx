...
export default function AudeMapPage() {
  ...
  return (
    <div className="max-w-7xl mx-auto p-4 bg-slate-50 min-h-screen">
      <nav className="mb-4">
        <Link href="/" className="inline-flex items-center gap-2 text-blue-800 font-bold hover:underline">
      ...
      <header className="mb-6">
        <div className="flex flex-col md:flex-row md:items-end justify-between gap-4">
          <div>
            <h1 className="text-2xl md:text-3xl font-extrabold text-slate-900 italic leading-tight">🏰 Sites Touristiques dans l'Aude (11)</h1>
            <div className="flex items-center gap-2 text-blue-700 font-bold text-sm mt-2">
              <Database size={16} />
      ...
      <div className="relative mb-6">
        ...
        <input 
          ...
          className="w-full pl-10 pr-4 py-3 rounded-xl border border-slate-200 focus:ring-2 focus:ring-blue-600 outline-none shadow-sm transition-all text-sm"
          value={searchQuery}
          ...
          <tbody className="divide-y divide-slate-100">
            {filteredSites.map((site, i) => (
              <React.Fragment key={`aude-${site.id}`}>
                ...
                {expandedId === i && (
                  <tr className="bg-blue-50/20 md:hidden">
                    <td colSpan={3} className="p-4 pt-0">
                      <div className="flex flex-col gap-3 py-4 border-t border-blue-100">
                        ...
                        <div className="flex items-center gap-2 text-slate-500">
                          <MapPin size={16} />
                          <span className="text-xs italic">Département de l'Aude (11)</span>
                        </div>
                      </div>
                    </td>
                  </tr>
