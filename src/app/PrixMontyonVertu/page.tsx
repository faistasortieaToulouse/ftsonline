...
interface Laureat {
  annee: number;
  nom: string;      // Correspond à "nom" dans ton JSON
  details?: string; // Correspond à "details" dans ton JSON
  lieu?: string;    // Correspond à "lieu" dans ton JSON
}

export default function PrixMontyonVertuPage() {
  const [laureats, setLaureats] = useState<Laureat[]>([]);
  const [loading, setLoading] = useState(true);

  useEffect(() => {
    fetch('/api/PrixMontyonVertu')
      .then((res) => res.json())
      .then((data) => {
        // On vérifie si les données sont dans l'objet "laureats"
        if (data && data.laureats && Array.isArray(data.laureats)) {
      ...
      <div className="max-w-4xl mx-auto">
        ...
        <div className="bg-white rounded-2xl shadow-xl overflow-hidden border border-purple-100">
          <div className="bg-purple-600 p-6 text-white flex items-center gap-4">
            ...
            <div>
              <h1 className="text-2xl font-bold uppercase tracking-tight">Prix Montyon de la Vertu</h1>
              <p className="text-purple-100 text-sm opacity-80">Palmarès des actes de dévouement</p>
            </div>
          </div>
            ...
            <table className="w-full text-left border-collapse">
              <thead>
                <tr className="bg-purple-50 text-purple-700 uppercase text-xs font-bold tracking-wider">
                  ...
                  <th className="p-4 border-b">Lauréat</th>
                  <th className="p-4 border-b">Action ou Détails</th>
                </tr>
              </thead>
                ...
                {laureats.map((l, index) => (
                  <tr key={index} className={`group transition-colors ${index % 2 === 0 ? 'bg-white' : 'bg-slate-50/50'} hover:bg-purple-50/50`}>
                    ...
                    <td className="p-4 font-semibold border-b border-slate-100">
                      {l.nom}
                      {l.lieu && <span className="text-xs font-normal text-slate-400 block italic">{l.lieu}</span>}
                    </td>
                    <td className="p-4 border-b border-slate-100 text-slate-600 text-sm">
                      {l.details ? l.details : <span className="text-gray-400">Dévouement reconnu</span>}
                    </td>
                  </tr>
                ))}
              </tbody>
