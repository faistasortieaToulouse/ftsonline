...
export default function Meteo2026Page() {
  const [data, setData] = useState<any[]>([]);
  const [error, setError] = useState(false);

  useEffect(() => {
    // On appelle l'API spécifique à 2026
    fetch("/api/meteo2026")
      .then((res) => {
  ...
  if (error) return <div className="p-10 text-center text-red-500 font-bold">Erreur de chargement des données 2026.</div>;
      ...
      <div className="max-w-6xl mx-auto">
        <header className="mb-8 text-center md:text-left">
          <h1 className="text-3xl font-extrabold text-slate-800 tracking-tight">
            Météo Toulouse <span className="text-indigo-600">2026</span>
          </h1>
          <p className="text-slate-500 mt-2 text-sm italic">
             Suivi en temps réel & Archives de l'année en cours
          </p>
        </header>
        ...
        {data.length === 0 && (
          <div className="mt-20 text-center animate-pulse">
            <p className="text-slate-400 font-bold tracking-widest uppercase text-xs">Analyse des données 2026...</p>
          </div>
        )}
      </div>
    </div>
