...
export default function MuseeAveyronPage() {
  ...
  return (
    <div className="max-w-7xl mx-auto p-4 bg-slate-50 min-h-screen font-sans">
      <nav className="mb-6">
        <Link href="/" className="inline-flex items-center gap-2 text-indigo-700 font-bold hover:underline group">
      ...
      <header className="mb-8">
        <h1 className="text-3xl font-black text-slate-900 leading-tight uppercase tracking-tight">
            🐑 Patrimoine de l'Aveyron (12)
        </h1>
        <p className="text-slate-500 font-medium italic mt-1">Des terrasses du Larzac aux clochers de Rodez</p>
      </header>
      ...
      <div className="relative mb-8">
        ...
        <input 
          ...
          className="w-full pl-12 pr-4 py-4 rounded-2xl border-none focus:ring-2 focus:ring-indigo-500 outline-none shadow-sm transition-all bg-white"
          value={searchQuery}
      ...
      >
        {!isMapReady && (
          <div className="absolute inset-0 flex flex-col items-center justify-center bg-slate-50/90 z-10 backdrop-blur-sm">
            ...
            <p className="text-indigo-600 font-black text-xs uppercase tracking-widest">Initialisation de la carte...</p>
          </div>
        )}
      </div>
      ...
      <h2 className="text-2xl font-black mb-6 text-slate-800 flex items-center gap-3">
        ...
        SITES ET MUSÉES ({filteredMusees.length})
      </h2>
            ...
            {filteredMusees.map((m, i) => (
              <React.Fragment key={`group-${i}`}>
                <tr 
                  ...
                  className={`cursor-pointer transition-colors scroll-mt-24 ${expandedId === i ? 'bg-indigo-50/50' : 'hover:bg-slate-50'}`}
                >
                  <td className="p-5 text-center font-black text-indigo-500 align-top">{i + 1}</td>
                  ...
                  <td className="p-5 align-top">
                    ...
                    <div className="text-[10px] text-indigo-600 font-black md:hidden mt-1 uppercase italic tracking-wider">{m.commune}</div>
                  </td>
                  <td className="p-5 hidden md:table-cell align-top text-center">
                    <span className="px-2 py-1 bg-indigo-100 text-indigo-700 rounded-lg text-[9px] font-black uppercase tracking-tighter">
                        {m.categorie}
                    </span>
                  </td>
                  ...
                  <td className="p-5 text-center align-top">
                    {m.url ? (
                      <a href={m.url} target="_blank" rel="noopener noreferrer" className="text-indigo-600 hover:scale-125 transition-transform inline-block" onClick={(e) => e.stopPropagation()}>
                ...
                {expandedId === i && (
                  <tr className="bg-indigo-50/30 md:hidden animate-in fade-in duration-300">
                    <td colSpan={3} className="p-5 pt-0">
                      <div className="flex flex-col gap-3 py-4 border-t border-indigo-100">
                        <div className="flex items-start gap-3 text-slate-600">
                          <Tag size={16} className="mt-0.5 text-indigo-400 flex-shrink-0" />
      ...
      <footer className="mt-20 py-10 border-t border-slate-200 text-center">
        <p className="text-[10px] font-black text-slate-300 uppercase tracking-[0.6em]">
            Aveyron Culture • 2026 • Rouergue Patrimoine
        </p>
      </footer>
    </div>
  );
