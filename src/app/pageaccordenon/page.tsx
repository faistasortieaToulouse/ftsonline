...
{/* Section des rubriques centrée */}
<section id="categories" className="py-8 px-4 max-w-3xl mx-auto">
    <h2 className="text-3xl font-bold mb-6 text-center text-purple-700">Nos rubriques</h2>
  
  <div className="space-y-4">
    {categories.map((cat, idx) => {
      const Icon = cat.icon;
      const sources = getSourcesForCategory(cat); // Utilise ta logique de sources ici

      return (
        <details key={idx} className="group bg-white rounded-2xl shadow-sm border border-gray-100 overflow-hidden transition-all">
          <summary className="flex items-center justify-between p-5 cursor-pointer list-none hover:bg-pink-50 transition-colors">
            <div className="flex items-center gap-4">
              <div className="p-2 bg-pink-100 rounded-lg text-pink-600">
                <Icon size={24} />
              </div>
              <span className="text-xl font-bold text-slate-800">{cat.title}</span>
            </div>
            
            {/* Le Triangle qui tourne à l'ouverture */}
            <div className="text-pink-500 transition-transform duration-300 group-open:rotate-180">
              <ChevronDown size={24} />
            </div>
          </summary>

          <div className="p-5 bg-slate-50 border-t border-gray-50">
            <p className="text-sm text-slate-500 mb-4 italic">
               {cat.isAgenda ? "Sélectionnez une source spécifique :" : `Explorer les ressources ${cat.title.toLowerCase()} :`}
            </p>
              ...
              {sources.length > 0 ? (
                sources.map((src: any) => (
                  <Link 
                    key={src.href} 
                    href={src.href} 
                    className="p-3 bg-white rounded-xl border border-pink-100 hover:border-pink-500 hover:shadow-md transition-all text-pink-700 font-medium"
                  >
                    • {src.title}
                  </Link>
                ))
              ) : (
                <Link href={cat.href} className="col-span-full p-3 bg-pink-600 text-white rounded-xl text-center font-bold">
                  Accéder à la rubrique principale
                </Link>
              )}
            </div>
          </div>
