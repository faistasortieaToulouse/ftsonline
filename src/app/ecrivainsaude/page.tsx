...
import { ecrivainsData, Ecrivain } from "@/data/ecrivainsAude";
...
export default function EcrivainsAudePage() {
  ...
  useEffect(() => {
    ...
    ecrivainsData.forEach((ecrivain) => {
      geocoder.geocode(
        { address: ecrivain.commune, region: "fr" },
        (results, status) => {
          ...
          const marker = new google.maps.Marker({
            ...
            title: ecrivain.nom,
            label: String(count), // simple, propre, stable
          });

          const info = new google.maps.InfoWindow({
            content: `
              <div style="font-family: Arial; font-size: 14px;">
                <strong>${count}. ${ecrivain.nom}</strong><br/>
                <b>Commune :</b> ${ecrivain.commune}<br/>
                <b>Dates :</b> ${ecrivain.dates || "N/A"}<br/>
                <b>Description :</b> ${ecrivain.description || "Écrivain"}
              </div>
            `,
          });
  ...
  return (
    <div className="p-4 max-w-7xl mx-auto">
      ...
      <h1 className="text-3xl font-extrabold mb-6">
        🖋️ Écrivains de l'Aude sur la carte
      </h1>
