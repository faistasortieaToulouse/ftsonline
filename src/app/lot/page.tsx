...
const LOT_CENTER: [number, number] = [44.4475, 1.4419];

const getThemeColor = (categorie: string): string => {
  const cat = categorie?.toLowerCase() || '';
  if (cat.includes('incontournable')) return '#ef4444'; // Rouge
  if (cat.includes('remarquable'))    return '#f97316'; // Orange
  if (cat.includes('suggéré') || cat.includes('suggere')) return '#3b82f6'; // Bleu
  return '#3b82f6';
};
...
export default function LotMapPage() {
        ...
        L.marker([site.lat, site.lng], { icon: customIcon })
          .bindPopup(`
            <div style="text-align: center; font-family: sans-serif; min-width: 140px;">
              ...
              <a href="#site-lot-${currentNum}" style="display: inline-block; background-color: ${color}; color: white; padding: 4px 10px; border-radius: 4px; text-decoration: none; font-size: 11px; font-weight: bold;">Détails ↓</a>
            </div>
          `)
  ...
  return (
    <div className="max-w-7xl mx-auto p-4 bg-slate-50 min-h-screen">
      <nav className="mb-4">
        <Link href="/" className="inline-flex items-center gap-2 text-stone-800 font-bold hover:underline">
      ...
      <header className="mb-6">
        <div className="flex flex-col md:flex-row md:items-end justify-between gap-4">
          <div>
            <h1 className="text-2xl md:text-3xl font-extrabold text-slate-900 italic leading-tight">🪨 Sites Touristiques dans le Lot (46)</h1>
            <div className="flex items-center gap-2 text-stone-700 font-bold text-sm mt-2">
              <Database size={16} />
      ...
      <div className="relative mb-6">
        ...
        <input 
          type="text"
          placeholder="Rechercher Rocamadour, Saint-Cirq-Lapopie, un gouffre..."
          className="w-full pl-10 pr-4 py-3 rounded-xl border border-slate-200 focus:ring-2 focus:ring-stone-600 outline-none shadow-sm transition-all text-sm"
          value={searchQuery}
        ...
        <table className="w-full text-left border-collapse text-sm table-fixed md:table-auto">
          ...
          <tbody className="divide-y divide-slate-100">
            {filteredSites.map((site, i) => (
              <React.Fragment key={`lot-${site.id}`}>
                <tr 
                  id={`site-lot-${i + 1}`}
                  onClick={() => setExpandedId(expandedId === i ? null : i)}
                  className={`cursor-pointer transition-colors scroll-mt-20 ${expandedId === i ? 'bg-stone-50/50' : 'hover:bg-slate-50'}`}
                >
                  ...
                  <td className="p-4 text-right md:text-left font-bold text-[13px] md:text-base whitespace-nowrap" style={{ color: getThemeColor(site.categorie) }}>
                    {site.categorie}
                  </td>
                </tr>

                {expandedId === i && (
                  <tr className="bg-stone-50/30 md:hidden">
                    <td colSpan={3} className="p-4 pt-0">
                      <div className="flex flex-col gap-3 py-4 border-t border-stone-100">
                        ...
                        <div className="flex items-center gap-2 text-slate-500">
                          <MapPin size={16} />
                          <span className="text-xs italic">Quercy / Vallée du Lot (46)</span>
                        </div>
                      </div>
                    </td>
                  </tr>
