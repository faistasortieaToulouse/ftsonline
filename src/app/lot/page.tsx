...
interface SiteLot {
  ...
  description: string;
  niveau: number;
  categorie: string; // On utilise string pour la souplesse de comparaison
  lat: number;
  lng: number;
} 
...
// --- Couleurs thématiques avec sécurité sur la casse ---
...
const getLabelColor = (categorie: string): string => {
  const cat = categorie?.toLowerCase() || '';
  return cat.includes('remarquable') ? 'white' : 'yellow';
};
...
export default function LotMapPage() { 
  ...
  useEffect(() => {
    async function fetchSites() {
      try {
        ...
        let data: SiteLot[] = await response.json();
  ...
  return ( 
    <div className="p-4 max-w-7xl mx-auto"> 
      ...
      <h1 className="text-3xl font-extrabold mb-6">🪨 Sites Touristiques dans le Lot</h1> 
        ...
        <table style={{ width: "100%", borderCollapse: "collapse", minWidth: "700px", backgroundColor: "white" }}> 
          ...
          <tbody> 
            {sitesData.map((site, i) => {
              const themeColor = getThemeColor(site.categorie);
              return (
                <tr key={site.id} style={{ backgroundColor: i % 2 === 0 ? "#ffffff" : "#f8fafc" }}> 
                  ...
                  <td style={{ ...tableCellStyleCenter, color: themeColor, fontWeight: 'bold' }}>
                    {site.niveau}
                  </td> 
                  <td style={{ ...tableCellStyle, color: themeColor, fontWeight: 'bold' }}>
                    {site.categorie}
                  </td> 
                </tr> 
              );
