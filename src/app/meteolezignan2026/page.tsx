...
// 1. Modification du nom de la fonction
export default function MeteoLezignan2026Page() {
  const [data, setData] = useState<any[]>([]);
  const [error, setError] = useState(false);

  useEffect(() => {
    // 2. Modification de l'appel API vers votre nouvel endpoint
    fetch("/api/meteolezignan2026")
      .then((res) => {
  ...
  if (error) return <div className="p-10 text-center text-red-500 font-bold">Erreur de chargement des données de Lézignan (2026).</div>;
      ...
      <div className="max-w-6xl mx-auto">
        <header className="mb-8 text-center md:text-left">
          <h1 className="text-3xl font-extrabold text-slate-800 tracking-tight">
            {/* 3. Modification du titre affiché */}
            Météo Lézignan-Corbières <span className="text-indigo-600">2026</span>
          </h1>
          <p className="text-slate-500 mt-2 text-sm italic">
             Suivi en temps réel & Archives locales (Aude)
          </p>
        </header>
                ...
                {data.map((d, index) => (
                  <tr key={index} className="hover:bg-indigo-50/30 transition-colors">
                    ...
                    <td className="p-4 text-center text-sm font-medium">
                      {d.vent.toFixed(0)} <span className="text-[10px] text-slate-400 font-normal">km/h</span>
                    </td>
                    <td className="p-4 text-center text-sm font-medium">
                        {d.pluie > 0 ? (
        ...
        {data.length === 0 && (
          <div className="mt-20 text-center animate-pulse">
            <p className="text-slate-400 font-bold tracking-widest uppercase text-xs">Synchronisation Lézignan 2026...</p>
          </div>
        )}
      </div>
    </div>
