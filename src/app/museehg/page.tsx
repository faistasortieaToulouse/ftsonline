...
export default function MuseeHGPage() {
  ...
  if (error) return <div className="p-10 text-blue-600 text-center font-bold italic">Erreur : {error}</div>;

  return (
    <div className="max-w-7xl mx-auto p-4 bg-slate-50 min-h-screen font-sans">
      <nav className="mb-6">
        <Link href="/" className="inline-flex items-center gap-2 text-blue-700 font-bold hover:underline group">
      ...
      <header className="mb-8">
        <h1 className="text-3xl font-black text-slate-900 leading-tight uppercase tracking-tight italic">
          🏰 Musées de la Haute-Garonne (31)
        </h1>
        <p className="text-slate-500 font-medium italic mt-1">
          {isLoadingData ? 'Chargement...' : `${filteredMusees.length} lieux d'exception entre Pyrénées et Canal du Midi.`}
        </p>
      </header>
      ...
      <div className="relative mb-8">
        ...
        <input 
          type="text"
          placeholder="Toulouse, Saint-Bertrand, Revel..."
          className="w-full pl-12 pr-4 py-4 rounded-2xl border-none focus:ring-2 focus:ring-blue-600 outline-none shadow-sm transition-all bg-white"
          value={searchQuery}
        ...
        <table className="w-full text-left border-collapse text-sm">
          <thead className="bg-slate-900 text-white">
            <tr>
              ...
              <th className="p-5 font-black uppercase text-[10px] tracking-widest">Établissement</th>
            ...
            {filteredMusees.map((m, i) => (
              <React.Fragment key={`group-${i}`}>
                <tr 
                  ...
                  className={`cursor-pointer transition-colors scroll-mt-24 ${expandedId === i ? 'bg-blue-50' : 'hover:bg-slate-50'}`}
                >
                  <td className="p-5 text-center font-black text-blue-700 align-top">{i + 1}</td>
                  ...
                  <td className="p-5 align-top">
                    ...
                    <div className="text-[10px] text-blue-600 font-black md:hidden mt-1 uppercase italic tracking-wider">{m.commune}</div>
                  </td>
                  <td className="p-5 hidden md:table-cell align-top text-center">
                    <span className="px-2 py-1 bg-blue-100 text-blue-800 rounded-lg text-[9px] font-black uppercase tracking-tighter">
                        {m.categorie}
                    </span>
                  </td>
                ...
                {expandedId === i && (
                  <tr className="bg-blue-50/30 md:hidden animate-in fade-in duration-300">
      ...
      <footer className="py-10 border-t border-slate-200 text-center">
        <p className="text-[10px] font-black text-slate-300 uppercase tracking-[0.6em]">
            Destination 31 • 2026 • Haute-Garonne Tourisme
        </p>
      </footer>
    </div>
  );
