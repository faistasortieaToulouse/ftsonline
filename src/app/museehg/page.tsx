...
const THEME_COLOR = '#1d4ed8'; // Bleu Haute-Garonne
...
export default function MuseeHGPage() {
  ...
  return (
    <div className="max-w-7xl mx-auto p-4 bg-slate-50 min-h-screen">
      <nav className="mb-4">
        <Link href="/" className="inline-flex items-center gap-2 text-blue-700 font-bold hover:underline">
          <ArrowLeft size={18} /> Retour à l'accueil
        </Link>
      </nav>

      <header className="mb-6">
        <h1 className="text-2xl md:text-3xl font-extrabold text-slate-900 leading-tight italic">🏰 Musées de la Haute-Garonne (31)</h1>
        <p className="text-slate-600 mt-1 font-medium italic">
          {isLoadingData ? 'Chargement...' : `${filteredMusees.length} sites culturels en Haute-Garonne.`}
        </p>
      </header>
      ...
      <div className="relative mb-6">
        ...
        <input 
          type="text"
          placeholder="Rechercher un musée, une ville (Toulouse, Saint-Bertrand...)"
          ...
          <tbody className="divide-y divide-slate-100">
            {filteredMusees.map((m, i) => (
              <React.Fragment key={`group-${i}`}>
                ...
                >
                  <td className="p-4 text-center font-bold text-blue-700 align-top">{i + 1}</td>
                  ...
                  <td className="p-4 align-top">
                    ...
                    <div className="text-[10px] text-blue-600 font-bold md:hidden mt-1 uppercase italic leading-none">{m.commune}</div>
                  </td>
