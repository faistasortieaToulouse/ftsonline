        ...
        L.marker([site.lat, site.lng], { icon: customIcon })
          .bindPopup(`
            <div style="text-align: center; font-family: sans-serif; min-width: 150px;">
              ...
              <a href="#site-hp-${currentNum}" style="display: inline-block; background-color: ${color}; color: white; padding: 4px 10px; border-radius: 4px; text-decoration: none; font-size: 11px; font-weight: bold;">Détails ↓</a>
            </div>
          `)
            ...
            {filteredSites.map((site, i) => (
              <React.Fragment key={`hp-${site.id}`}>
                <tr 
                  ...
                  className={`cursor-pointer transition-colors scroll-mt-20 ${expandedId === i ? 'bg-sky-50/30' : 'hover:bg-slate-50'}`}
                >
