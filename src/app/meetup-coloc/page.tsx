...
"[https://via.placeholder.com/400x200?text=Événement+Meetup](https://via.placeholder.com/400x200?text=Événement+Meetup)";
...
`${process.env.NEXT_PUBLIC_BASE_URL}/api/meetup-coloc`,
{ cache: "no-store" }
);

if (!res.ok) throw new Error("Impossible de récupérer les événements Meetup");

const data = await res.json(); // ← le JSON GitHub est déjà un tableau
...
console.error(err);
error =
err.message || "Erreur inconnue lors du chargement des événements Meetup";
}

return ( <div className="container mx-auto py-10 px-4 sm:px-6 lg:px-8"> <h1 className="text-3xl font-bold mb-4">
Évènements Meetup Toulouse colocation, logement, emploi, café des
langues </h1> <p className="text-muted-foreground mb-6">
