...
interface PodcastEpisode {
  ...
  description: string;
  audioUrl: string;
  source: "Ombres Blanches" | "Terra Nova" | "Marathon des Mots";
}

export default function PodLibrairiesPage() {
  ...
  const filterEpisodes = useMemo(
    () =>
      debounce(() => {
        ...
        if (sourceFilter !== "All") {
          filtered = filtered.filter(ep => ep.source === sourceFilter);
        }

        setFilteredEpisodes(filtered);
      }, 300),
    [episodes, search, sourceFilter]
  );
  ...
  }, [search, sourceFilter, filterEpisodes]);
  ...
  return (
    <div className="container mx-auto py-10 px-4 min-h-screen bg-gray-50">
      ...
      <div className="flex flex-col sm:flex-row items-stretch sm:items-center gap-4 mb-10 p-4 bg-white rounded-xl shadow-lg border border-gray-100">
        ...
        <select
          value={sourceFilter}
          onChange={e => setSourceFilter(e.target.value as PodcastEpisode["source"] | "All")}
        ...
        >
          {loading ? 'Chargement...' : '🔄 Rafraîchir le cache'}
        </button>
      </div>
