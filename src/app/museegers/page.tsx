  ...
  return (
    <div className="max-w-7xl mx-auto p-4 bg-slate-50 min-h-screen">
      ...
      <header className="mb-6">
        <h1 className="text-2xl md:text-3xl font-extrabold text-slate-900 leading-tight italic">🍷 Musées et Patrimoine du Gers (32)</h1>
        <p className="text-slate-600 mt-1 font-medium italic">
          {isLoadingData ? 'Chargement...' : `${filteredMusees.length} sites touristiques gascons.`}
        </p>
      </header>
      ...
      <div className="relative mb-6">
        ...
        <input 
          type="text"
          placeholder="Rechercher une bastide, un musée, une commune..."
          className="w-full pl-10 pr-4 py-3 rounded-xl border border-slate-200 focus:ring-2 focus:ring-red-500 outline-none shadow-sm transition-all"
          value={searchQuery}
          ...
          <tbody className="divide-y divide-slate-100">
            {filteredMusees.map((m, i) => (
              <React.Fragment key={`group-${i}`}>
                ...
                >
                  <td className="p-4 text-center font-bold text-red-700 align-top">{i + 1}</td>
                  ...
                  <td className="p-4 align-top">
                    ...
                    <div className="text-[10px] text-red-600 font-bold md:hidden mt-1 uppercase italic leading-none">{m.commune}</div>
                  </td>
