        ...
        {loading ? (
          ...
          <div className="space-y-8">
            <div className="bg-purple-100 p-4 rounded-xl text-purple-800 font-medium flex items-center gap-2">
              <Info size={18} />
              Résultat : {data.count} événement(s) trouvé(s) pour Toulouse.
            </div>
            ...
            {data.events?.map((event: any, i: number) => (
              <div key={i} className="bg-white rounded-3xl shadow-sm border border-slate-100 overflow-hidden transition-all hover:shadow-md">
                ...
                <div className="p-6 md:p-8">
                  ...
                  <div className="mb-8">
                    <h3 className="text-sm font-bold uppercase tracking-wider text-slate-400 mb-3">À propos de l'événement</h3>
                    <div className="text-slate-600 text-sm leading-relaxed whitespace-pre-wrap bg-slate-50 p-4 rounded-2xl border border-slate-50">
                      {event.description}
                    </div>
                  </div>
                  ...
                  <a 
                    ...
                    className="flex items-center justify-center gap-2 bg-purple-600 text-white w-full py-4 rounded-2xl font-bold hover:bg-purple-700 transition-all shadow-lg shadow-purple-100"
                  >
            ...
            {data.count === 0 && (
              <div className="text-center p-16 bg-white rounded-[2.5rem] border-2 border-dashed border-slate-200 text-slate-400">
                ...
                <p className="text-sm">Le filtrage fonctionne, mais aucun événement du flux ne mentionne "Toulouse".</p>
              </div>
            )}
          </div>
        )}
