...
/* =========================
    COMPOSANT PRINCIPAL
========================= */
export default function HierarchieSunniteArabePage() {
  const [arbre, setArbre] = useState<NoeudSunnite[]>([]);
  const [loading, setLoading] = useState(true);
  const positions: Position[] = [];

  useEffect(() => {
    fetch("/api/hierarchieSunniteArabe")
      .then((res) => res.json())
  ...
  return (
    <main className="p-4 md:p-8 min-h-screen bg-[#fcfaf7]">
      ...
      <nav className="mb-8 max-w-7xl mx-auto">
        <Link href="/" className="inline-flex items-center gap-2 text-emerald-900 hover:text-emerald-700 font-bold group transition-colors">
      ...
      {loading ? (
        <div className="text-center py-20 animate-pulse text-emerald-900 font-serif italic uppercase text-xs tracking-widest">
          Ouverture des manuscrits anciens...
        </div>
      ) : (
        <div className="max-w-7xl mx-auto grid grid-cols-1 xl:grid-cols-[1fr_420px] gap-8 items-start">
          ...
          >
            <div className="bg-emerald-50/50 border-b border-emerald-100 p-5 flex items-center gap-3">
              ...
              <h3 className="text-emerald-900 font-black uppercase text-xs tracking-widest">Jurisprudence et Doctrine</h3>
            </div>
                ...
                {positions.map((p, i) => (
                  <g key={p.noeud.id}>
                    ...
                    <foreignObject x={p.x} y={p.y} width={LARGEUR_NOEUD} height={HAUTEUR_NOEUD} className="overflow-visible">
                      <div className="h-full w-full bg-white border-2 border-emerald-900 rounded-2xl p-4 shadow-lg flex flex-col justify-center text-center relative hover:scale-105 transition-transform duration-300">
      ...
      <footer className="mt-12 text-center text-[10px] text-slate-400 uppercase tracking-[0.3em]">
        Patrimoine et Tradition Arabe — Études Islamiques
      </footer>
    </main>
  );
}