...
export default function HierarchieChiitePersePage() {
  ...
  useEffect(() => {
    fetch('/api/hierarchieChiitePerse')
      .then(res => res.json())
      .then((data: ChiitePerse[]) => {
        // Attribution propre des IDs et maintien de l'ordre hiérarchique
  ...
  return (
    <main className="p-8 min-h-screen bg-[#f7f9f7]">
      ...
      <header className="mb-16 text-center max-w-7xl mx-auto">
        ...
        <p className="text-slate-500 italic font-serif">Structure spirituelle et académique d'Iran et d'Irak</p>
        ...
        <div className="max-w-7xl mx-auto grid grid-cols-1 xl:grid-cols-2 gap-12 items-start">
          ...
          <div className="order-2 xl:order-1 bg-white border-l-4 border-l-emerald-700 rounded-r-2xl shadow-xl overflow-hidden border-y border-r border-slate-200">
            <div className="bg-slate-50 border-b p-4 flex items-center gap-2">
              ...
              <h3 className="text-slate-800 font-bold uppercase text-xs tracking-widest">Registre des Savants</h3>
            </div>
          ...
          <div className="order-1 xl:order-2 bg-[#f0f4f0] border-2 border-slate-200 rounded-3xl p-8 overflow-auto max-h-[750px] shadow-inner relative">
              ...
              {positions.map((p, i) => (
                <g key={i}>
                  ...
                  >
                    ...
                    ">
                      {/* Badge de Rang */}
                      <div className="absolute -top-3 left-1/2 -translate-x-1/2 bg-emerald-700 text-white text-[10px] font-black px-4 py-1 rounded-full shadow-md border border-emerald-500">
                        RANG {p.noeud.id}
                      </div>
                      
                      {/* Section (ex: Clergé Supérieur) */}
                      <p className="text-[10px] text-emerald-600 font-bold uppercase mb-1 tracking-widest break-words w-full px-2">
                        {p.noeud.section}
                      </p>
                      
                      {/* Titre (ex: Grand Ayatollah) */}
                      <h4 className="text-[15px] font-black text-slate-900 leading-tight break-words w-full px-4">
                        {p.noeud.titre}
                      </h4>
                    </div>
                  </foreignObject>
