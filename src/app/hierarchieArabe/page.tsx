...
import { ArrowLeft, Landmark, ScrollText, Compass } from "lucide-react";
...
export default function HierarchieArabePage() {
  ...
  useEffect(() => {
    fetch('/api/hierarchieArabe')
      .then(res => res.json())
      .then((data: HierarchieArabe[]) => {
        // Attribution des IDs pour garantir que le Calife/Sommet soit le N°1
  ...
  return (
    <main className="p-8 min-h-screen bg-[#faf9f6]">
      ...
      <header className="mb-16 text-center max-w-7xl mx-auto">
        ...
        <p className="text-slate-500 italic font-serif">Structure des titres et fonctions de l'Empire</p>
        <div className="mt-4 flex justify-center items-center gap-2">
          <span className="h-[1px] w-20 bg-amber-200"></span>
      ...
      {loading ? (
        ...
        <div className="max-w-7xl mx-auto grid grid-cols-1 xl:grid-cols-2 gap-12 items-start">
          ...
          {/* TABLEAU DES NIVEAUX */}
          <div className="order-2 xl:order-1 bg-white border-l-4 border-l-amber-600 rounded-r-2xl shadow-xl overflow-hidden border-y border-r border-slate-200">
            <div className="bg-slate-50 border-b p-4 flex items-center gap-2">
              <ScrollText size={18} className="text-amber-700" />
              <h3 className="text-slate-800 font-bold uppercase text-xs tracking-widest">Registre des Titres</h3>
            </div>
                ...
                <tbody className="divide-y divide-slate-100">
                  {personnes.map((p) => (
                    <tr key={p.id} className="hover:bg-amber-50/30 transition-colors">
                      <td className="px-6 py-4 text-center font-bold text-amber-800 bg-amber-50/50">{p.id}</td>
                      <td className="px-6 py-4">
                        <div className="font-bold text-slate-900 leading-tight">{p.titre}</div>
                        <div className="text-[10px] text-amber-600 font-semibold uppercase mt-1">{p.section}</div>
                      </td>
          ...
          {/* ARBRE VISUEL SVG */}
          <div className="order-1 xl:order-2 bg-[#fcfaf7] border-2 border-amber-100 rounded-3xl p-8 overflow-auto max-h-[750px] shadow-inner relative">
              ...
              {positions.map((p, i) => (
                <g key={i}>
                  ...
                  >
                    <div className="
                      min-h-[110px] w-full 
                      bg-white border-2 border-amber-50 rounded-xl 
                      p-4 shadow-md flex flex-col items-center justify-center 
                      text-center transition-all hover:border-amber-500 hover:shadow-lg relative
                    ">
                      <div className="absolute -top-3 left-1/2 -translate-x-1/2 bg-amber-600 text-white text-[10px] font-black px-4 py-1 rounded-full shadow-md border border-amber-400 uppercase">
                        Rang {p.noeud.id}
                      </div>
                      
                      <p className="text-[10px] text-amber-700 font-bold uppercase mb-1 tracking-widest break-words w-full px-2">
                        {p.noeud.section}
                      </p>
                      
                      <h4 className="text-[16px] font-black text-slate-900 leading-tight break-words w-full px-4">
                        {p.noeud.titre}
                      </h4>
                    </div>
                  </foreignObject>
