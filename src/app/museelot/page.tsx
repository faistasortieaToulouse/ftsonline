...
const THEME_COLOR = '#b91c1c'; // Rouge Malbec
    ...
    <div className="max-w-7xl mx-auto p-4 bg-slate-50 min-h-screen font-sans">
      ...
      <header className="mb-8">
        <h1 className="text-3xl font-black text-slate-900 leading-tight uppercase tracking-tight italic">
          🍷 Patrimoine du Lot (46)
        </h1>
        <p className="text-slate-500 font-medium italic mt-1">
          {isLoadingData ? 'Chargement du Quercy...' : `${filteredMusees.length} escales culturelles de Cahors à Rocamadour.`}
        </p>
      </header>
      ...
      <div className="relative mb-8">
        ...
        <input 
          type="text"
          placeholder="Rechercher à Cahors, Figeac, Saint-Cirq-Lapopie..."
      ...
      >
        {!isMapReady && (
          <div className="absolute inset-0 flex flex-col items-center justify-center bg-slate-50/90 z-10 backdrop-blur-sm">
            ...
            <p className="text-red-600 font-black text-xs uppercase tracking-widest">Génération de la carte...</p>
          </div>
        )}
      </div>

      <h2 className="text-2xl font-black mb-6 text-slate-800 flex items-center gap-3">
        <span className="h-1.5 w-12 bg-red-700 rounded-full"></span>
        LES SITES À VISITER
      </h2>
        ...
        <table className="w-full text-left border-collapse text-sm">
          <thead className="bg-slate-900 text-white">
            <tr>
              ...
              <th className="p-5 hidden md:table-cell w-48 font-black uppercase text-[10px] tracking-widest text-center">Type</th>
          ...
          <tbody className="divide-y divide-slate-100">
            {filteredMusees.map((m, i) => (
              <React.Fragment key={`group-${i}`}>
                ...
                >
                  ...
                  <td className="p-5 align-top">
                    <div className="flex items-center gap-2">
                      ...
                      <div className="md:hidden text-red-400">
                        {expandedId === i ? <ChevronUp size={18} /> : <ChevronDown size={18} />}
                      </div>
                    </div>
                    <div className="text-[10px] text-red-700 font-black md:hidden mt-1 uppercase italic tracking-wider">{m.commune}</div>
                  </td>
                  ...
                  <td className="p-5 text-center align-top">
                    <a href={m.url} target="_blank" rel="noopener noreferrer" className="text-red-700 hover:scale-125 transition-transform inline-block" onClick={(e) => e.stopPropagation()}>
                ...
                {expandedId === i && (
                  <tr className="bg-red-50/30 md:hidden animate-in fade-in duration-300">
      ...
      <footer className="py-10 border-t border-slate-200 text-center">
        <p className="text-[10px] font-black text-slate-300 uppercase tracking-[0.6em]">
            Oh My Lot ! • 2026 • Tourisme en Occitanie
        </p>
      </footer>
    </div>
  );
