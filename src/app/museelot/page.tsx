// src/app/museelot/page.tsx
'use client';

import { useEffect, useState, useRef, useCallback } from 'react';
// Importe le type depuis l'API du Lot
...
const GoogleMap = ({ musees }: { musees: Musee[] }) => {
  ...
  const initMap = useCallback(() => {
    ...
    // Calculer le centre de la carte (Lot - estimation basée sur la moyenne des coordonnées)
    ...
    const map = new window.google.maps.Map(mapRef.current, {
      ...
      zoom: 9, // Zoom initial pour le Lot
      scrollwheel: true,
    });
...
export default function MuseeLotPage() {
  ...
  useEffect(() => {
    async function fetchMusees() {
      try {
        // Appel à l'API du Lot
        const response = await fetch('/api/museelot'); 
        if (!response.ok) {
          throw new Error("Erreur lors de la récupération des données de l'API du Lot.");
        }
  ...
  if (isLoading) {
    return (
      <div style={{ padding: '20px' }}>
        <h1>Musées et Patrimoine du Lot (46)</h1>
