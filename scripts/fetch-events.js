...
const fetch = require("node-fetch"); // si Node 18+, fetch est natif, tu peux supprimer cette ligne
...
const fetchFrenchTechRSS = async () => {
  ...
  try {
    ...
    return feed.items.map((item, i) => ({
      ...
      name: item.title?.trim() || item["dc:title"]?.trim() || "Événement sans titre",
      date: item.isoDate || new Date().toISOString(),
      location: "Lieu à définir",
      description: item.contentSnippet || item.content || "Pas de description.",
      image: item.enclosure?.url || item["media:content"]?.url || PlaceHolderImages[i % 4].imageUrl,
...
const fetchOpenData = async () => {
  ...
  try {
    ...
    return (json.records || []).map((r, i) => {
      ...
      return {
        id: f.uid || `hg-${i}`,
        name: f.title?.trim() || f.nom?.trim() || "Événement sans titre",
        ...
        image: f.image_url || PlaceHolderImages[i % 4].imageUrl,
        imageHint: PlaceHolderImages[i % 4].imageHint,
      };
    });
  } catch (e) {
...
const fetchToulouseMetropole = async () => {
  ...
  try {
    ...
    return (json.records || []).map((r, i) => {
      ...
      return {
        id: f.id_manif || `tm-${i}`,
        name: f.titre?.trim() || f.nom?.trim() || "Événement sans titre",
